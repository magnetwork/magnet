...
{
    // Convert magnet:// to magnet:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("magnet://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "magnet:");
    }
...
{
    return GetAutostartDir() / "magnet.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a magnet.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the magnet app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add magnet app to startup item list
    ...
    QMessageBox(parent)
{
    header = tr("Magnet-Qt") + " " + tr("version") + " " +
        ...
        tr("Usage:") + "\n" +
        "  magnet-qt [" + tr("command-line options") + "]                     " + "\n";
