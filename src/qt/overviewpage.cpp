...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMagnetAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMagnetAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMagnetAmount = strAnonymizeMagnetAmount.remove(strAnonymizeMagnetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMagnetAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMagnetAmount*COIN) nMaxToAnonymize = nAnonymizeMagnetAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMagnetAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMagnetAmount));
        strAnonymizeMagnetAmount = strAnonymizeMagnetAmount.remove(strAnonymizeMagnetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMagnetAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
